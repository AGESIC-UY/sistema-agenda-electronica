...
//			List<ValorPosible> valoresDTO = obtenerValoresPosibles(datoASolicitar);
	...
	public void copiarRecurso(Recurso r, String nombre, String descripcion) throws BusinessException, ApplicationException, UserException {
		...
		if (existeRecursoPorNombre(rCopia) ) {
			throw new UserException("-1","Ya existe el recurso de nombre "+ rCopia.getNombre());
		}
		if (existeRecursoPorDescripcion(rCopia) ) {
			throw new UserException("-1","Ya existe el recurso con descripción "+ rCopia.getDescripcion());
		}
		...
		trCopia.setTituloCiudadanoEnLlamador(r.getTextoRecurso().getTituloCiudadanoEnLlamador());
		trCopia.setTituloPuestoEnLlamador(r.getTextoRecurso().getTituloPuestoEnLlamador());
